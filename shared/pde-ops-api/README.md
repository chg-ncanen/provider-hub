...
```bash
cd provider-hub/shared/pde-ops-api
pip install -e .
```

The `pde-mcp` MCP server (`provider-hub/plugins/pde-ops-tools/mcp-servers/pde-mcp/`) depends on this package via its `requirements.txt` rather than bundling a copy — install that instead to get both:

```bash
cd provider-hub/plugins/pde-ops-tools/mcp-servers/pde-mcp
...
this package's own location — so run it from `plugins/pde-ops-tools/mcp-servers/pde-mcp/` (where this
...
- `plugins/pde-ops-tools/mcp-servers/pde-mcp/` — MCP server exposing these APIs as tools
- `plugins/pde-ops-tools/skills/resolve-duplicate-contact-alerts/` — Skill using JSM operations

## Dependencies

See `requirements.txt` in `plugins/pde-ops-tools/mcp-servers/pde-mcp/`.
