...
```json
{
  "mcpServers": {
    "pde-mcp": {
      "command": "python",
      "args": ["/path/to/provider-hub/plugins/pde-ops-tools/mcp-servers/pde-mcp/app.py"]
    }
  }
}
```
...
- `shared/pde-ops-api` (repo root) — Shared API layer for JSM operations; a `requirements.txt`
