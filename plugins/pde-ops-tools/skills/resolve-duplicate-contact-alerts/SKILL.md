...
**Prerequisites:** `mcp-servers/pde-mcp/.env` with `ATLASSIAN_EMAIL` + `ATLASSIAN_API_TOKEN`, optional `EMAIL_*` vars for email check, and `sf` CLI authenticated to the `prod` org alias. This script runs directly, not through `.mcp.json`, so it never sees Claude Code's `userConfig` substitution the way `pde-mcp` itself does — on Claude Code, the plugin's `SessionStart` hook mirrors `userConfig` into this `.env` for exactly this reason, so it should just be there after `/plugin configure pde-ops-tools@provider-hub` + a session restart. On Copilot CLI (no `userConfig`), create it by hand.
