...
if [ -n "${CLAUDE_PLUGIN_OPTION_ATLASSIAN_EMAIL:-}${CLAUDE_PLUGIN_OPTION_ATLASSIAN_API_TOKEN:-}${CLAUDE_PLUGIN_OPTION_REPOS_DIR:-}${CLAUDE_PLUGIN_OPTION_CONFLUENCE_SPACE_KEY:-}${CLAUDE_PLUGIN_OPTION_CONFLUENCE_PARENT_PAGE_ID:-}${CLAUDE_PLUGIN_OPTION_CONFLUENCE_SYNC_ENABLED:-}" ]; then
  {
    ...
    # already-empty file.
    if [ -f "$ENV_FILE" ]; then
      grep -vE '^(ATLASSIAN_EMAIL|ATLASSIAN_API_TOKEN|REPOS_DIR|CONFLUENCE_SPACE_KEY|CONFLUENCE_PARENT_PAGE_ID|CONFLUENCE_SYNC_ENABLED)=' "$ENV_FILE" || true
    fi
