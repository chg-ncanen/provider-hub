...
if [ -n "${CLAUDE_PLUGIN_OPTION_ATLASSIAN_EMAIL:-}${CLAUDE_PLUGIN_OPTION_ATLASSIAN_API_TOKEN:-}${CLAUDE_PLUGIN_OPTION_REPOS_DIR:-}" ]; then
  {
    ...
    # already-empty file.
    if [ -f "$ENV_FILE" ]; then
      grep -vE '^(ATLASSIAN_EMAIL|ATLASSIAN_API_TOKEN|REPOS_DIR)=' "$ENV_FILE" || true
    fi
