...
Whether a newly-opened PR starts as a draft is controlled by
`DRAFT_PR_ENABLED` (this plugin's `userConfig`) — default **on**: the ticket's
assignee gets first look before anyone else is notified or requested to
review. A human marks it ready for review in GitHub whenever they're
satisfied; nothing in this pipeline flips it automatically (`ticket-merge`
treats a still-draft PR as pending, not blocked — see its Step 1).
...
# DRAFT_PR_ENABLED isn't set as a real Bash env var by Claude Code — it's
# mirrored into $CLAUDE_PLUGIN_ROOT/.env by the bootstrap-env.sh SessionStart
# hook, same as every other userConfig option this skill relies on. Default
# on unless explicitly turned off.
DRAFT_PR_SETTING="${DRAFT_PR_ENABLED:-${CLAUDE_PLUGIN_OPTION_DRAFT_PR_ENABLED:-}}"
if [ -z "$DRAFT_PR_SETTING" ] && [ -f "$CLAUDE_PLUGIN_ROOT/.env" ]; then
  DRAFT_PR_SETTING=$(grep -m1 '^DRAFT_PR_ENABLED=' "$CLAUDE_PLUGIN_ROOT/.env" | cut -d= -f2-)
fi
DRAFT_FLAG="--draft"
case "$(echo "${DRAFT_PR_SETTING:-true}" | tr '[:upper:]' '[:lower:]')" in
  false|0|no) DRAFT_FLAG="" ;;
esac
...
if [ -z "$EXISTING_PR" ]; then
  ...
  # than creating a duplicate. Its draft state is whatever it already is;
  # this setting only governs PRs created fresh, right above.
